...
一个盘的「根目录」指的是这个盘之下的第一级目录（恰好是「树」之「根」），例如 C 盘根目录就指的是路径 `C:\` 。
...
不过，不是所有压缩软件的解压操作都叫「提取」。如果没有看到「提取」的字眼，不妨找找「解压」或者「解压缩」之类的词，它们的作用也是类似的。
...
4. 如果你不小心把一个用户文件夹的位置设置成了某个磁盘的根目录（比如说 `D:\`），会触发 Windows 系统的一个 bug，导致很难再改回来。~~（辣鸡 Windows）~~