...
- 一个光秃秃的 `exe` 文件。
- 一个压缩包，例如 `zip` 或 `rar` 文件。
- 一个扩展名是 `iso` 的文件。

对于第一种情况，我们直接双击这个 `exe` 文件就能启动安装进程。对于第二种情况，我们需要解压缩这个压缩包到某处，然后在解压出来的文件中找到名字类似「`setup.exe`」或「`install.exe`」的程序双击打开。对于第三种情况，我们右击这个 `iso` 文件，选择【打开方式】→【文件资源管理器】，然后在弹出的新窗口中找到名字类似「`setup.exe`」或「`install.exe`」的程序双击打开[¹](software-installation.md)。

启动安装器后，我们一般按提示【下一步】操作即可完成安装。

不过在此过程中也要留心，跨过了「高速下载器」的坎，可不要又掉进了捆绑软件的坑。**有一些软件在安装程序中也会像「高速下载器」一样勾选了一些捆绑软件、浏览器主页劫持什么的，这些选项可能出现在安装过程中的任何阶段，一定要注意取消勾选再进行下一步。**
...
`Program Files` 和 `Program Files (x86)` 这两个文件夹，即是系统预置的两个用来安装 app 的文件夹，都位于 C 盘根目录下。然而，我们希望把软件安装到 D 盘。这里推荐一个简单好用的方法：**直接将默认路径中的 `C:` 改成 `D:`。**例如：
...
- 一种是已经完全破解了的收费软件。这种软件已经经过修改，安装包往往也是民间自行制作的，可以直接走正常流程安装，安装后打开就可以无限制地使用。例如 [vposy](https://weibo.com/vposy) 所制作的 Adobe 软件套装。
