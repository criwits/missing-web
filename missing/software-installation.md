...
然而，**对于百度这样的搜索引擎，官网常常不是搜索结果中的第一个——百度搜索出来的前 3 条结果一般是广告。**例如，我们想要下载「WPS」，直接在百度中搜索「WPS 下载」，我们会得到这样的搜索结果：
...
- 一个光秃秃的 `exe` 文件或者 `msi` 文件。
- 一个压缩包，例如 `zip` 或 `rar` 文件。
- 一个扩展名是 `iso` 的文件。

对于第一种情况，我们直接双击这个文件就能启动安装进程。对于第二种情况，我们需要解压缩这个压缩包到某处，然后在解压出来的文件中找到名字类似「`setup.exe`」或「`install.exe`」的程序双击打开。对于第三种情况，我们右击这个 `iso` 文件，选择【打开方式】→【文件资源管理器】，然后在弹出的新窗口中找到名字类似「`setup.exe`」或「`install.exe`」的程序双击打开[¹](software-installation.md)。
