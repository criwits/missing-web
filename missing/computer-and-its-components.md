...
![左：台式电脑的 CPU 芯片；右：笔记本电脑的 CPU 芯片](computer-and-its-components/Untitled.png)

左：台式电脑的 CPU 芯片；右：笔记本电脑的 CPU 芯片
...
![左：英特尔的旧贴纸；右：英特尔的新贴纸](computer-and-its-components/Untitled%201.png)
...
我们现在把处理器可以比做成上面例子中的「老师」：「老师」的作用是对「作业」进行批改，从而完成教学任务；处理器的作用是对数据进行处理，从而实现各种各样的功能。
...
![上：台式电脑中的内存条；下：笔记本电脑中的内存条](computer-and-its-components/RAMs.jpg)
...
![左：一块机械硬盘；右：一块固态硬盘](computer-and-its-components/Disks.jpg)
...
![左：典型的 Windows 系统的界面；右：典型的 macOS 的界面。](computer-and-its-components/Untitled%204.png)

左：典型的 Windows 系统的界面；右：典型的 macOS 的界面。
...
![如上图所示，右击「此电脑」选择「属性」来查看自己电脑 Windows 的版本。（这里展示的是 Windows 10）](computer-and-its-components/Untitled%205.png)
...
![使用「任务管理器」查看硬盘的数量和类型。](computer-and-its-components/Untitled%206.png)
