...
- 不小心安装了不想要的软件，怎么卸载它们？
- 为什么打开有些软件时，系统总是提示「你要允许此应用对你的设备进行更改吗」？
...
![诱导重新安装的按钮](basic-maintenance/Recommending_others_during_uninstallation.png?updated=20250911#center)
...
当然，这一套权限系统是有很多空子的。提升的权限具有从属关系——如果某个程序被提权，那么由它打开的其他程序也默认具有特权。因而，如果你为一个不怀好意的程序赋予了特权，那它就可以为所欲为了，因为它能够不动声色地给其他程序特权，然后做一些不好的事情。所以，最终我们还是需要控制住自己电脑上软件的来源，不要让「不干净」的软件来到我们的电脑上。因此，**不运行来源不明的可执行文件**，是保证安全的根本；即使不得不使用来源未知的程序时，也要先使用安全软件对它们进行扫描。
