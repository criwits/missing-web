...
RAR 格式的压缩率通常比 ZIP 低不少。除此之外，它还支持一系列其他的 ZIP 所没有的功能，例如「恢复记录」和更加安全的加密功能。如果你的硬盘空间不够做一份备份，或需要网络传输大量文件，那么可以考虑利用 RAR 的恢复记录功能来防止数据损坏。但鉴于制作 RAR 格式的压缩包只能使用 WinRAR，你需要自行作出权衡。 
...
当你使用 Bandizip 打开一个压缩文件时，软件界面大致如下图，你可以在此对压缩文件进行各种操作。对于前文说到的 ZIP 文件编码问题，Bandizip 也有一定的自动检测功能，但如果它检测失败了，你也可以点击右上方的【🌐】来手动选择正确的编码。
...
PeaZip 是老牌开源压缩软件，初版发布于 2006 年，但界面与时俱进，拥有现代风格的简洁美。它的主界面如下图所示，看起来有点像文件资源管理器，实际上你也确实能用它来浏览其他文件，但仅限浏览。PeaZip 同样支持读取几乎所有格式的压缩包，也能制作 ZIP、7z 等格式的压缩包。

![PeaZip 的主界面](archive-formats-and-tools/PeaZip.png#center)

作为一款开源软件，PeaZip 能够借助社区的力量实现一些好用的功能。它拥有类似 Bandizip 的智能解压功能，也能够快捷设置 ZIP 文件编码，不过无法自动检测。然而，PeaZip 不支持 Windows 11 的新式菜单[^peazip_menu]，也就是说，它在新式右键菜单中没有任何选项，所有选项均只位于旧式菜单中。

[^peazip_menu]: 其实 PeaZip 提供了一些注册表文件让你在新式右键菜单添加选项，但原理是抢占原本 Windows 给一些系统应用准备的位置，着实不太优雅。

PeaZip 的官网是 [https://peazip.github.io/](https://peazip.github.io/)，进入之后根据你使用的操作系统来选择对应的下载链接。你也可以到它的 GitHub 地址 [https://github.com/peazip/PeaZip/releases](https://github.com/peazip/PeaZip/releases) 下载安装。但需注意：**安装程序并不会根据你电脑使用的语言来设置 PeaZip 的语言，并且安装完毕后无法更改**。所以你需要在安装过程中手动设置语言，多少有些不便。

![在 PeaZip 安装时设置语言](archive-formats-and-tools/PeaZip_Change_Lang.png#center)
