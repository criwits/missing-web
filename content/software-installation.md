...
> 往上面两个位于 C 盘的路径放东西是需要管理员权限的（见 [基本维护和安全防护]({{<relref "basic-maintenance.md#应用的权限与-uac-弹窗">}})）。为了避免提权，有些软件会默认安装到你用户文件夹下的 `AppData` 文件夹，即
>
> ```
> C:\用户\<你的用户文件夹名>\AppData
> ```
> 
> 相比于前面的位置，在 `AppData` 中放东西不需要管理员权限。但是，`AppData` 文件夹默认是隐藏的，并不好找，这可能给我们管理软件安装位置带来一些麻烦。
...
{{<hint warning>}}
注意：如果你想把软件安装到除 C 盘以外的地方，那么按上面的做即可；如果你想把软件安装到 C 盘，那么请不要动安装路径；如果你想把软件安装到 C 盘里面的其他地方，我劝你别。

当然，之前提到的装在 `AppData` 下的软件同样可以照猫画虎地「改 C 为别的」来安装到其他分区，但我们不推荐这样，因为会增加管理难度。
{{</hint>}}
