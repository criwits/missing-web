...
> 此外，常用的压缩格式 ZIP 也有易出现乱码的毛病——ZIP 不保证在压缩文件中记录文件名所采用的编码方式，因此，当使用 ZIP 格式跨语言环境（如从中文版 Windows 传输到英文版 Windows）或跨操作系统（如从 Windows 电脑传输到手机上）转移文件时，有可能出现乱码问题。而那些内置编码记录的压缩格式，如 7z 或 RAR 等，则不存在此问题。
