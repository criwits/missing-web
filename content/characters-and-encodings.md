...
可惜，许多时候我们并没有「选择使用什么编码体系」的权力，这时就只能选择去「适应环境」了。你或许在阅读一些软件的教程时，看见过「不要使用中文路径」「不要使用中文用户名」这样的说法，这么做也是为了防止编码不同造成问题。在 Windows 平台上，软件内部会使用五花八门的编码来存储数据（包括文件路径、用户数据等），这些数据在我们的中文版 Windows 中可能被用 GB18030 解码[^evil-party]，若编码不匹配，软件就会找不到文件甚至直接崩溃。另一方面，一些老旧的外国软件则默认使用 Windows-1252 等欧美地区常用的单字节编码，这些软件胡乱解码 GB18030 编码出来的多字节数据时，亦将产生错误。如果只使用英文路径，由于几乎所有编码都对 ASCII 兼容，我们就可以避免这样的问题。
