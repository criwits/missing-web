...
> 此外，常用的压缩格式 ZIP 也有容易出现乱码的毛病——ZIP 不保证会在压缩文件中记录文件名所采用的编码方式。因此，当使用 ZIP 格式跨语言（比如从中文版 Windows 传输到英文版 Windows）或跨系统（比如从 Windows 电脑传输到手机上）转移文件时，就有可能出现乱码问题。而那些内置编码记录的压缩格式（如 7z 或 RAR 等）就不存在这种问题。
