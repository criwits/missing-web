...
可惜，许多时候我们并没有「选择使用什么编码体系」的权力，这时就只能选择去「适应环境」了。你或许在阅读一些软件的教程时，看见过「不要使用中文路径」「不要使用中文用户名」这样的说法，这么做也是为了防止编码不同造成问题：在 Windows 平台上，有些软件内部使用 UTF-8 来编码文字内容，包括文件路径、用户数据等。当这些软件运行在采用 GB18030 的中文版 Windows 上时，可能误用 GB18030 来解码这些内容[^evil-party]，从而找不到文件甚至直接崩溃。另外，还有一些老旧的外国软件则默认用户使用 Windows-1252 等欧美地区常用的单字节编码，它们无法正确处理使用 GB18030 编码出来的多字节数据，亦会产生错误或无法运行。如果只使用英文路径，由于几乎所有编码都对 ASCII 兼容，我们就可以避免这样的问题。

[^evil-party]: 实际上 Windows 内部还使用 UTF-16 编码字符串，这可能让本就错误的编码雪上加霜。

> 相比于 Windows，Linux 和 macOS 就没有这样的顾虑：自本世纪初开始，这两款操作系统便在实践中全面采用 UTF-8 来处理文件路径等信息。相比之下，Windows 为了保持对旧版本系统和软件的兼容性，迟迟未能全面实现向 Unicode 和 UTF-8 的转移。
>

既然 UTF-8 可以同时支持几乎所有语言，保证无论在哪都不会出现乱码，那能不能也让 Windows 系统抛掉其他代码页，完全拥抱 UTF-8 呢？诶，Windows 的确提供了这样的选项，但是藏得比较深。如下图所示，你得转到系统设置的「语言和区域」，然后点【管理语言设置】→【更改系统区域设置】才能找到。但是、然而、可惜，如你所见，微软宣称这个选项只是「Beta 版」，也就是处于「不怎么能用」的状态。开启这个选项后，UTF-8 会成为系统默认的「代码页」，可以在一定程度上解决跨语言环境下的程序乱码问题。但是，那些不支持多字节编码的老软件仍然会无法使用，同时更多难以预料的兼容性问题亦会浮出水面。所以我们在此处只是展示「有这么个选项」，如果遇到编码问题，除非别无他法，否则不要启用。~~（看来有时历史包袱过重也会阻碍新事物发展呢。）~~
