...
「为 Unicode 字符集编码」这个事情，需要一点创造力。像 GB 18030 一样直接使用字符集的字符码肯定是不行的，因为 Unicode 字符集的字符码覆盖了 0ʜ~10FFFFʜ 的每个自然数，而直接用会引起歧义，不妨以「U 盘」这个词为例，把它每个字符的字符码写出来连在一起，就是 `55 76 D8`，但是要想还原回去就难了：

![Unicode直接编码](characters-and-encodings/UnicodeEncoding.png)

任意长的字符码让我们无法知道哪里是字符的开始，哪里又该是这个字符的结束。

那不如……把编码弄成定长的，而且长度要能容下最大的字符码吧？这就是 UTF-32 所想的。师承 ISO 的编码方案，UTF-32 使用 32 位二进制数（4 字节）来编码字符，要知道，最大的 Unicode 码位 10FFFFʜ 也才 21 位二进制数，这显然绰绰有余，做到一字符一编码。于是，在 UTF-32 下，「U 盘」一词的编码就是 `00 00 00 55 00 00 76 D8`，真长啊。相信你也看出来了，UTF-32 的弊端就是占空间，存储大多数文本的时候，数据有一半多会是纯纯的 `0`，多浪费啊。所以，这种编码方式并没有推广开来。
...
> 相比于 Windows，Linux 和 macOS 就没有这样的顾虑。自 2000 年开始，UTF-8 被各 Linux 发行版采用为系统级的默认编码；而早在 1998 年，Mac OS 引入了 HFS+ 文件系统，使用 UTF-16 来进行文件名的存储，并自 2005 年的 Mac OS X 10.4 Tiger 开始在系统级全面采用 UTF-8。相比之下，Windows 或许是为了保持对旧版本系统和软件的兼容性，迟迟未能全面实现向 Unicode 和 UTF-8 的转移。
>
