...
这样，世界各地的计算机可以通过代码页来确定应采用的编码规范。例如，对于英语内容，它们使用代码页 38，通过 ASCII 解码；而对于简体中文内容，则采用代码页 936，对应国家标准《GB/T 2312-1980 信息交换用汉字编码字符集 基本集》（简称「GB2312」）这套编码。相较于为所有语言统一编码，代码页则要求为每段内容明确指定其编码，从而使计算机能够自动切换解码方式以正确读取。由于每个地区的计算机通常只需处理本地语言内容，操作系统会设置一个「默认代码页」。当内容未明确标注其代码页时，系统会默认使用该默认代码页进行解码。
...
> 此外，常用的压缩文件格式 ZIP 也存在容易出现乱码的毛病——ZIP 不保证在压缩文件中记录文件名所采用的编码方式，因此，当使用 ZIP 格式跨语言环境（如从中文版 Windows 传输到英文版 Windows）或跨操作系统（如从 Windows 电脑传输到手机上）转移文件时，有机率出现乱码问题。一些内置编码记录的压缩格式，如 7z 或 RAR 等格式，则不存在此问题。
...
既然 UTF-8 可以保证几乎无论在哪都不会出现乱码，那能不能也让 Windows 系统抛掉代码页，彻底拥抱 UTF-8 呢？诶，Windows 的确提供了这样的选项，但是藏得比较深。如下图所示，你得转到系统设置的「语言和区域」，然后点【管理语言设置】→【更改系统区域设置】才能找到。但是、然而、可惜，如你所见，微软宣称这个选项只是「Beta 版」，也就是处于「不怎么能用」的状态。开启这个选项后，所有软件都会默认使用 UTF-8 编码，但不支持 UTF-8 的应用（例如一些较老的应用程序）会乱码甚至无法使用。所以我们在此处只是展示「有这么个选项」，如果遇到编码问题，除非别无他法，否则不要启用。~~（看来有时历史包袱过重也会阻碍新事物发展呢。）~~
