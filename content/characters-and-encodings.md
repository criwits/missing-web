...
> 所谓「私用区」（Private Use Area，简称 PUA）是指码表中专门保留的一段码位，这些码位不会被分配给具体的字符。换句话说，如果你需要「自造字」，就可以将这些码位分配给你自己创造的文字，从而**在你自己的系统中**使用这些字符。在早期中文编码尚未覆盖大量生僻字的情况下，我国公安系统曾利用 PUA 为未编码的姓名生僻字分配码位，使这些字能够录入系统。然而，随着 GB18030 和 Unicode 标准的不断发展，越来越多的生僻字获得了正式的编码，这导致了「一字多码」的现象：某个生僻字既有公安系统使用的 PUA 码位，又有正式编码。这种情况引发了一些实际问题，比如有人发现自己的身份证在实名验证时无法通过，原因是身份证内的生僻字使用的是 PUA 编码，但是该字已经有了通行的正式的编码，计算机因为二者编码不一致而判定为不同的名字。为解决这一问题，目前 [我国官方和社会各界正在积极推动相关措施](https://name.vurls.cn/ui/index/index/)。
