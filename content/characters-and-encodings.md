...
但是原本 Unicode 设计的是「每个字符 16 位二进制编码」，也就是 65536 个字符，我们早已知道这不够用。Unicode 联盟意识到这个问题之后，引入了类似 ISO 当初设想的「平面」（plane）概念，但远没有 ISO 方案那么复杂。他们将 Unicode 的字符范围分为 0~16 号总共 17 个平面，每个平面正好 16 位编码，65536 个字符，这样算下来就有{{<katex>}}16 \times 65536 = 1\,114\,112{{</katex>}}个码位，这下够用了。
