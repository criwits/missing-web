...
与此同时，他们对工厂内的设备进行了改造。对于那些用电脑控制的机床等设备，他们安装了收集数据和远程控制的软件，这些软件会将机器运转的完整情报，实时上传到云服务器上。而对于那些不便更换的老旧设备，则被加装了各种传感器——传感器能采集一部分诸如温度、震动、转速的信息。这些传感器连接到一台台「微电脑」上，数据亦被传送到云端。当工厂运行时，各式各样的生产数据被上传到远方的服务器上，经过分析和处理，呈现在她和同事面前。而当生产存在问题时，算法亦能识别出数据之下的隐患，让她和同事们能够及早介入，减小损失。
...
在「她」的故事中，工厂大大小小的设备，通过安装软件、加装传感器和微电脑等方式，实现了数据的在线收集和上传。在这之后，这些工业设备就连入了网络。借助云计算带来的廉价算力，这些来自边缘的设备得以被有效地连接。

> 这里的「微电脑」，指的是一种特别小型的计算机。它们性能很差，但在结构上确实是完整的计算机，拥有处理器、内存和类似硬盘的部件，并且通常具有连接网络的功能。这些微电脑可以做到一张长宽仅几厘米的小电路板上，可以被轻松地「嵌入」各种各样的机器内部，使它们成为「物联网」的一部分，因此又称为「嵌入式计算机」。与它们相关的软件、硬件开发则被称为「嵌入式开发」。
