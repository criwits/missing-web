...
第二次世界大战期间，纳粹德国使用一种被称为「ENIGMA」（意为「谜」）的密码机来对军事通讯进行加密。这种密码机形如一台打字机，只不过它没法装纸，并不能在纸上「打字」；相反地，它多了一个由 26 盏小灯泡组成的显示板，每个灯泡都与键盘上的 26 个字母对应。此外，在机器上方，它还装有几个印有字母的机械转盘（称为转子）。下方左侧就是一种常见型号的 ENIGMA 密码机的示意图。当使用者在键盘上击键时，某个其他字母对应的小灯就会亮起——这便是对应字母加密后的密文。

![ENIGMA](introduction-to-cryptology/Enigma.png#center)

上方右图则展示了 ENIGMA 密码机的工作原理。每次按下一个键 ❶ 后，转子就会转动 ❷ 一格。转子之间有联动机构——当一个转子转完一圈后，它左边的转子也会转一格。同时，按下的键发出一个电信号，并首先被传到「接线板」，在这里，使用者可以安装一些「跳线」，将信号对换 ❸ 到其他的字母上。紧接着，电信号进入 ❹ 第一个转子。不同的转子内部会按各自的规则将字母进行替换，使用者可以按需选用不同型号的转子。信号依次流过 ❺ 三个转子，并在折返后再反向通过它们。此时，电信号对应的字母已经和原来按下的字母几乎没有关联了。最后，信号再被送入 ❻ 接线板进行最后一次替换 ❼，然后点亮对应字母的灯泡 ❽。
 
在这样的一整个流程中，字母一共经过了 8 次替换，而且由于转子每次击键都会旋转，这种替换关系在每个字母后都会变化，使得最终打出的文字与原文看起来毫无关系，字母之间的对应关系变得极其复杂。当需要解密消息时，只需要在另一台相同型号的密码机上安装相同的转盘，调整到相同的起始位置，并以相同方式连接跳线，即可打出原始的明文。许多历史学家认为，德军在战争前期的节节胜利和 ENIGMA 密码机不无关系，而盟军最终在西欧战场提前取胜，也大大得益于 ENIGMA 密码机的成功破译。
...
无论是凯撒密码还是 ENIGMA 密码机，它们都有一个共同的特征：**加密和解密过程使用相同的密钥**。对于凯撒密码，只要知道加密时使用的偏移量就能解密；而对于 ENIGMA 密码机，则需要知道加密时使用的转子型号、转子起始位置以及跳线连接情况。如同现实生活中的钥匙和锁一样，我们使用同一把钥匙来上锁和开锁。这样的密码被称为「对称密码」。对称密码是最基本的一种密码形式，它符合我们对「加密」这件事的直觉，因此当密码学从古典时期迈向近代时，人们首先研究的，便是设计出安全、高效的对称密码。
...
优秀的密码算法，就是科学地将混淆和扩散进行结合，实现最大的安全性。顺着这样的思路，我们介绍两款在密码学历史上具有重要意义的对称加密算法：曾经风光无限但已寿终正寝的 DES，以及今天正在广泛使用的 AES。
...
> 在本章后续的介绍中，「位」均指「二进制位」，比如「64 位的明文」指明文由 64 个二进制数字（即「比特」）组成。

而在 DES 对每一块加密的过程中，都会使用 56 位长度的密钥来**循环进行包含代换和置换的操作多达 16 次**。在这其中的代换部分，DES 使用一套预先设计好的代换表（称为「S 盒」）进行。例如，使用如下的 S 盒时，输入数据 46（二进制 `100110`），就能得到代换后的结果 8。DES 一共有 8 个这样的 S 盒 {{<katex>}}S_1{{</katex>}} 至 {{<katex>}}S_8{{</katex>}}，它们互不相同，每个都是经过特定设计得到的，可以在加密过程中引入复杂性来防止被轻易破解。
