...
你如果正在电脑上使用 Chrome 浏览器，可以按下 F12，在右上方找到【隐私和安全】（或者【Privacy and Security】）选项卡，就会发现这一句话：
...
- ECDHE 是一种基于椭圆曲线的密钥交换算法，它有点像我们提到的非对称加密算法如 RSA，也有成对的公钥与私钥，不过它专门被用来在通信双方交换后续加密的密钥——也就是接下来提到的 AES 算法的密钥；而 ECDSA 则是基于椭圆曲线的数字签名算法，我们将在后文详细介绍数字签名的作用。
- AES_128_GCM 则代表以 128 位为密钥长度的 AES 加密算法。当收到由之前由 ECDHE 传输过来的密钥之后，浏览器就有能力解密通过 AES 加密后传输过来的真正的网页内容，随后送给浏览器渲染并呈现在你眼前。
