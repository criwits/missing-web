...
「签名」，当今社会成本最低而最为常用的认证技术。上小学时，老师会要求我们作业写完要给家长签字，以表示家长真的监督过；或是把我们的考卷带回去家长签字，以代表家长真的看过了试卷 ~~上的分数~~。这「家长签名」正是一种认证，因为小学生往往不太可能模仿出家长的飘逸签名，老师也可以通过对比已有签名的方法将坏孩子「绳之以法」。在这二重保险之下，多数学生就只能选择「乖乖就范」~~而痛失快乐童年~~。

除了签名，盖章、按指印等也是常用的认证技术，广泛应用在下至日常上至国家机要的各种场合。但是，签名只来源于签名者、公章只有备案的才是真的……它们只能由待认证方产生，却可以被任何人检验——又一种非对称性。那么，用类似的思路来看网络世界的认证，就可以仿照非对称加密来完成「赛博签名」。

「赛博签名」中也有公钥与私钥，对应「只能由待认证方产生」的东西，是私钥，于是相应的验证工具就是公钥。使用私钥为一些信息签名，就相当于宣称「这些信息由我（私钥所有者）而来」，其他人只需通过公钥验证之，就能判断真假。无法伪造的密钥造就了一种「赛博签名」，也是解决「可认证性」问题的曙光。这就是「数字签名」（digital signature），或「公钥数字签名」，后文中出现的「签名」都指的是它。

回到《你缺计课》网页版的验证问题。有了数字签名，服务器（签名者）就可以用自己手里的私钥给内容签名，将签名与内容一同传输到你的设备。接下来，你的设备（或其他人）就能用服务器的公钥验证签名，也就能确定内容的真实性。这就是「验签」的过程，如下图所示。

> 和手写签名不同，电子签名与信息内容有关。同一个人给不同信息产生的签名必须不同，不然签名就没有意义了，它可以被其他人拿来「移花接木」。但是，信息内容本身可能很长，为了让签名变得简短，我们又该怎么让签名「和信息相关」呢？（提示：哈希。）请自行思考如何高效签名，并尝试优化下面的过程。
>

![Signing](introduction-to-cryptology/Signing.png#center)

显然，实现签名技术，就像非对称加密一样，依然离不开从那些单向陷门函数里寻找灵感。下面列出一些目前常用的签名算法，相信这里有许多你认识的面孔。
...
现在，问题似乎已经解决了。我们只需要再准备一对签名使用的密钥即可。这下，攻击者就算想伪装成我们，也伪造不出我们的签名——私钥牢牢掌握在我们手中。一切都很完美……
...
> 当然，我们可以选择再对公钥签一次名，这样问题就变成了「确认『用来确认「用来确认内容真实性的公钥」的真实性的公钥』的真实性」——这不[俄罗斯套娃](https://baike.baidu.com/item/%E4%BF%84%E7%BD%97%E6%96%AF%E5%A5%97%E5%A8%83/17215)嘛！一层层可以套到天荒地老。我们必须突破这个模式，寻找一个终结它的方法。
>
...
现在，让我们来试试把「证书」的操作应用到数字签名的过程中。现实的证书，证明的是「签名样本」的真实性，而它是用来验签的；但在数字签名中，无需签名样本，我们直接使用公钥验签。那么，让信用大户再来给公钥签个名，不就完事了吗？

又回到《你缺计课》的例子。如果存在一个你和《你缺计课》都信任的信用大户，我们就可以在向你的浏览器提供内容、签名和公钥的同时，再补充一份「信用大户对我们公钥的签名」作为「证书」。你的浏览器在收到这个证书后，可以用神秘人士的公钥验证它的有效性，进而就验证了我们的公钥的真实性。随后，一切就能像刚刚那套操作一样展开了——借助公钥验签，进而认证安全。就像这样：
