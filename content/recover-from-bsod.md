...
蓝屏死机，英文叫 BSoD（blue screen of death，「死亡蓝屏」，很形象啊）。我们熟知的 Windows 10 和早期 Windows 11 操作系统，它们的蓝屏错误界面很有特色，甚至成为了某些流行文化的一部分（可惜 Windows 11 在 2025 年末把它换掉了，详见后文），大概长这样：

![Windows 10/11 蓝屏](recover-from-bsod/Win-10-BSoD.png#center)

以一个大大的文字表情「 :( 」开头，代表了人们遇到这个界面时的心声，接下来是一些说明性文字，看起来很简洁。略览这些文字，我们不难推测，**蓝屏死机说明操作系统遇到某种极为严重的错误，被迫终止运行**。但最初的蓝色错误界面（事实上是如今蓝屏错误的前身）与这可有天壤之别。
...
Windows 2000 的后继者便是大名鼎鼎的 Windows XP，在前者发布的次年便很快发布，名字中的「XP」代表「e<u>xp</u>erience」（体验）。正如其名，Windows XP 为用户带来了不同于以往 NT 系列的体验，包括令人耳目一新的用户界面与大量性能改进。这个系统实在过于经典，以至于大概二十年后的现在，仍有一些重要设备在运行它。
...
在 2010 年左右的这段时间，微软想进军移动市场，顺便来一个移动桌面平台「大一统」（虽然目前来看，结果不怎么样，参见 [软件的寻找与安装]({{<relref "software-installation.md#uwp-应用和-microsoft-store">}})）。在此理念影响下，微软在 2012 年发布的 Windows 8 更像是一个平板电脑操作系统，操作逻辑与之前的 Windows 相去甚远，甚至连开始按钮都惨遭舍弃。用户当然不买账：「跟了我们这么久的开始按钮怎么能说丢就丢呢？」于是微软在随后的 Windows 8.1 把开始按钮加了回来，但也仅此而已，未能改变它「看起来像给平板用的系统」的本质，愿意用 Windows 8 系列的用户也还是不多。
...
Windows 10 的下一代是 2021 年发布的 Windows 11。值得一提的是，它的蓝屏界面在某个时段的预览版中变成了黑色，就像这样：

![某些 Windows 11 预览版的「蓝」屏](recover-from-bsod/Win-11-Pre-BSoD.png#center)

嗯……怎么说呢，看起来死气沉沉的。虽然之后发布的正式版改回了和 Windows 10 一模一样的界面，但这股「死气沉沉」之风在 2025 年下半年回归了，甚至愈演愈烈。这段时间的某次更新后，Windows 11 的蓝屏又变成了黑屏，并且「:(」消失了，只剩下了少得可怜的几行英文，颇有「墓志铭」的风格。~~（干脆改名叫「black screen of death」算了。）~~

![2025 年下半年后 Windows 11 的「蓝」屏](recover-from-bsod/Win-11-BSoD-late-2025.png#center)
...
> 如果在遇到蓝屏后，`C:\Windows\Minidump` 下没有产生转储文件，则需要按后文的方法修改系统转储设置。
...
{{<hint warning>}}
注意：不推荐优先使用本节的方法排查错误，此处只是为了展示「遵从提示」的情况。
{{</hint>}}

虽然较新的 Windows 11 的「蓝」屏界面已经没有多少信息，但从 Windows 10 与 早期 Windows 11 的蓝屏界面中我们还是能看到微软给用户的一丁点指引。
...
好咯，那我们就去看看。用手机扫码或在电脑上手动访问这个页面，如今，2026 年初，映入我们眼帘的是这样的页面：

![微软官方的简易操作指南](recover-from-bsod/Stopcode-Webpage-20260309.png?updated=20260309)

嗯……看起来只是在介绍「蓝屏错误是什么」。

再往下翻，你会发现微软列出的解决方案包括「删除新硬件」「在安全模式下启动」「更新驱动程序」等，事实上……也没错，解决蓝屏确实要修正点什么东西，但问题在于这「解决方案」只告诉我们类似「如何卸载软件」的方法论，却不告诉我们「应该卸载什么软件」之类的具体措施。归根结底还是造成蓝屏的原因千变万化，不能简单一概而论。所以微软也在教你如何「碰运气」。

而在这个页面的最底下，微软给出了这样一句话：

{{<hint quoting>}}

如果你是 IT 专业人员或高级用户，可以尝试 Windows 蓝屏、黑屏和停止代码错误的高级故障排除步骤。

{{</hint>}}

但是截至 2026 年 3 月 9 日，这句话附带的链接点进去后会让你无限重复登录你的微软账号……不愧是你，微软。事实上，这个链接应该指向微软的 [《停止代码错误的高级故障排除》](https://learn.microsoft.com/zh-cn/troubleshoot/windows-client/performance/stop-code-error-troubleshooting) 文章，这篇文章也介绍了一些 WinDbg 的使用方法，有兴趣的读者可前去一阅。
