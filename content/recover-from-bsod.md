...
自 1985 年起的几年，微软除了开发自家的 Windows 之外，还在与 IBM 合作开发一款名叫「OS/2」的操作系统，这个系统不是很出名，毕竟它的目标是 IBM 自家的计算机。到了 1988 年 11 月，微软的一个开发团队准备改进当时的 OS/2，使之成为一个兼容 POSIX（可移植操作系统接口）的多用户操作系统，起名「NT OS/2」[^1]。
...
1. 按下 `Windows` + `Pause` 或 `Windows` + `Break`，或者右击【此电脑】→【属性】来打开「系统信息」页面；
...
- 小内存转储：大小仅有 64 KB 到 256 KB，包含了最基本的崩溃进程、线程、内核堆栈数据与已加载的驱动程序列表，提供最基础的崩溃信息。它的文件名以 `MMDDYY-NNNNN-XX.dmp` 为格式，`MMDDYY` 是「月日年」格式的日期，`XX` 是两位数的文件编号。
