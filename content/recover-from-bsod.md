...
- 小内存转储：大小仅有一般在 256 KB 到几 MB 不等，包含了最基本的崩溃进程、线程、内核堆栈数据与已加载的驱动程序列表，提供最基础的崩溃信息。它的文件名以 `MMDDYY-NNNNN-XX.dmp` 为格式，`MMDDYY` 是「月日年」格式的日期，`XX` 是两位数的当天文件编号。
...
其中偏下方的位置有用醒目的蓝色写出的 `!analyze -v`，结合它旁边的说明就能知道，**运行这个命令就可以分析转储文件**。WinDbg 也很方便，**点击这里的蓝色文字就可以运行命令**。在命令执行过程中，左侧中下部的状态会显示 `*BUSY*`，同时最下方会显示正在从服务器下载哪些所需的文件。这时可以喝杯茶，等待分析过程完成。不过如果你发现下载过程实在太慢了，可以考虑[通过合法渠道进行国际互联网连接](https://www.12377.cn/xzfg/2020/7f441527_web.html)来加快下载速度。
