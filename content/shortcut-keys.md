...
- `Ctrl` + 鼠标点击：多选。在资源管理器中，按住 Ctrl 并依次点击多个文件或文件夹，可以同时选中它们；而在一些办公或设计类软件中，也可以用这个操作选中多个对象。不过，并非所有软件的「多选」都使用 `Ctrl` 键，如果你发现它不奏效，可以试试 `Shift` 或 `Alt`。

  > 顺带一提，在资源管理器中，按住 Shift 并依次点击两个文件或文件夹，可以一次性选中它们之间的所有文件（包括它们本身）。

- `Ctrl` + `A`：全选。在资源管理器中，按这个快捷键可以全选所有的文件和文件夹；而在编辑文本时，它可以选中当前输入框内所有的文本。例如，当你在 QQ 中想删掉已经打出来的所有内容时，只需按一次 `Ctrl` + `A`，再按 `Backspace` 就能完成这个操作。
- `Ctrl` + `D`：删除文件。仅适用于资源管理器，相当于按一次 `Delete`。
- `Ctrl` + `Shift` + `N`：在资源管理器（包括桌面）中新建文件夹。
- `Ctrl` + `F`：搜索。这个快捷键可用于大多数文本编辑器（例如「记事本」或 Word），按这个快捷键可以调出文本搜索的对话框。此外，在浏览器中，这个快捷键可以调出页内查找框，不妨现在就试试！
