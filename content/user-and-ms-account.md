...
在我们第一次启动 Windows 系统（参见 [笔记本新机「开荒」指南]({{<ref "new-laptop-setup.md">}}) ）时，系统会要求我们新建一个用户，并设置它的用户名和密码，它将作为你自己所使用的用户²。打开 `C:\用户\` 目录，你可以看到一个以它命名的文件夹，以及一个 `公用` 文件夹：
...
1. 注意，由于微软的疏忽，Windows 中多用「帐户」一词，然而正确的写法应是「账户」，但为了忠于原文，我们未作更改。
