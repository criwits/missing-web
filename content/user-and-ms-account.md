...
> 注意，由于微软的疏忽，Windows 中多用「帐户」一词，然而现在正确的写法应是「账户」。但本文为忠于原文，未更改相关词汇。
...
到此为止，我们都在说「我们可以创建的用户」。但其实系统之中，除了我们创建的用户之外，还存有一个**隐藏的「超级用户」**——Administrator（直译是「管理员」）。这个用户和我们创建的管理员不同，它**只有一种权限层级——管理员权限**。所以，它无论何时都有非常高的权限：更改系统关键设置、安装/卸载软件、创建新的用户、删除别的用户……甚至还可以「窥探」其他用户的文件。由于 Administrator 的权限太高、法术太强，Windows 系统便把它「封印」起来——它不可见、不可登录，甚至不可感知。而它的作用，一般是在我们自己的用户出现意料之外的故障时，用来救急。
...
你是否想过这样一个问题：如果我的电脑被坏人偷走了，那么他们能读取我电脑硬盘中的文件，窥探我的隐私吗？在过去很长一段时间中，这个问题的答案是肯定的——长期以来，Windows 都将文件明文存储在电脑硬盘上。即使用户设置了自己的「登录密码」，若我们将硬盘拆下，连接到别的电脑上，依然可以畅通无阻地读取其中的内容。也就是说，用户的登录密码只是一道防止攻击者进入 Windows 系统的「[马奇诺防线](https://baike.baidu.com/item/%E9%A9%AC%E5%A5%87%E8%AF%BA%E9%98%B2%E7%BA%BF/1713498)」，对数据安全并没有实质性的保护。

为了实现真正的安全，从 Windows 10 开始，一种名为「设备加密」的功能被添加到了系统中[^devsbitlocker]。这功能启用后，硬盘上的所有数据都被加密存储，若没有用于解密的密钥，外人无法读取出硬盘中的任何内容。密钥存储在电脑内部的一个名为「受信任的平台模块」（Trusted Platform Module，简称 TPM）的安全芯片中。TPM 会在电脑开机时检查系统安全情况——例如，开机过程中有无恶意软件作祟，电脑是否有被恶意开启并篡改过设置等。当这个芯片确认安全后，它就会向系统通报密钥并进行解密。整个过程中，用户看不到密钥。
...
不过，在某些极端情况下，TPM 可能产生误判。一旦它认为启动环境不安全，就会拦截启动过程，不再向系统通报密钥。这时你将会在开机时卡在类似下图的画面：

![BitLocker 恢复界面](user-and-ms-account/RecoverPass.png#center)

可是，我们作为普通用户，从来没见过那长长密钥的真面目啊。这时，如果你使用的是微软账号来登录系统，那么微软（一般）会自动帮你把它备份到云端。你只需要打开 [https://account.microsoft.com/devices/recoverykey](https://account.microsoft.com/devices/recoverykey) 这个链接，按提示登录微软账号，即可找到自己设备所有加密分区的，长达 48 个数字的密钥。~~虽然但是，谁愿意手打那么长的密钥啊。~~

![在微软账号网页上查看 BitLocker 密钥](user-and-ms-account/BitLocker_keys_in_MS_Account_page.png#center)

那么，如何查看自己电脑是否已经启用了设备加密功能呢？只需按键盘上的 `Windows` + `S`，输入「设备加密设置」几个字。如果你搜索不到任何设置选项（只能搜索到网上的在线内容），说明你的电脑不支持设备加密，更没有启用设备加密。不支持设备加密的原因有很多：比如，没有 TPM 或者 TPM 不受支持、未开启「安全启动」、部分硬件不受支持等。

![没有设备加密设置](user-and-ms-account/No_Device_Encryption.png#center)

如果你能搜索到名为【设备加密设置】的设置项目，说明你的电脑支持设备加密功能。点击该项，你可以看到当前设备加密的工作状态。

![查看设备加密设置](user-and-ms-account/Find_Device_Encryption.png#center)

如果你启用了设备加密，但是电脑没有登录微软账号，电脑上的设备加密就会处于「还差最后一块拼图」的状态，这「最后一块拼图」就是将密钥上传到微软账号。在这种情况下，你可以看到【使用你的 Microsoft 帐户登录以完成此设备的加密】的提示。

![设备加密警告](user-and-ms-account/Not_complete_encryption.png#center)

在这种情况下，系统有可能已经完成了硬盘的加密工作，但密钥无法上传备份，因此以某种明文形式[^clearkey]暂存在本地，硬盘处于「虽然被加密了，但是任何人都可以解密」的状态。只有我们登录自己的微软账号后，明文形式的密钥才会被销毁，设备加密的完整功能才得以施展。

这个过程看似非常完美，但 ~~由于微软越来越差的代码质量~~ 可能出现一些非常严重的问题——有时，用户还没有登录微软账号，又或者已经登录但未成功上传密钥，明文密钥也可能先于一步被销毁。在这种情况下，如果某次开机时 TPM 产生误判，认为启动存在不安全的因素，我们就有可能在开机时陷入「输入恢复密钥」的尴尬场面——此时密钥无处可寻，只能尝试多次重启、排查造成芯片认为不安全的因素来尝试进入系统。你可以在社交媒体上搜索「BitLocker 密钥」或「设备加密密钥」，就能看到这种情况屡见不鲜。

为了防止这种情况发展，如果你发现自己电脑启用了设备加密，请务必正确登录自己的微软账号，并且在前文中提到的查看密钥的链接中确认所有分区的密钥均已成功上传。如果你发现密钥没有成功上传，可以尝试在系统上退出（选择【改用本地帐户登录】）后重新登录微软账号。或者，如果你认为自己不需要设备加密功能，也可以在设置中直接将它关闭。

[^clearkey]: 称为「Clear Key」。
...
不过，仍然有一些事情可能需要我们处理。如果你照 [文件与文件管理]({{<relref "file-and-file-management.md#合理利用用户文件夹">}}) 中的方法更改了用户文件夹的存储位置，那么，你可能会发现自己的文件不见了，这是因为系统没有为新的用户文件夹路径记录存储位置。虽说文件还在之前改过的存储位置那里放着，但还是需要重新操作一遍更改。至于我们新建的那个临时管理员用户，你可以在设置中将它删除。
