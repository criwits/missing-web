...
[^multiuser]: 不过，在学校、企业、科研机构等场所，「多人共用一台高性能电脑」的情况仍然非常普遍。这种高性能的电脑通常被称为「服务器」，一般保持 24 小时运行，人们通过「远程连接」的方式来使用它们。服务器一般不运行 Windows 系统，而是使用 Linux 系统。
...
但 Windows 为我们提供了一套「提升权限」的机制。Administrator 这个重量级用户是不能亲自下场的，但我们可以把自己「提升到和它一致的权限」来做事。还记得前文提到，右击一个程序选择【以管理员身份运行】，就可以赋予程序提升的权限吗？所谓的「以管理员身份运行」，就是「提升到与 Administrator 一致的身份运行」，而 Administrator 有着很高的权限。那个「UAC 弹窗」的本质，就是某个程序在尝试让我们提升权限来执行自己时，系统弹出的警告。一旦我们选择了【是】，我们的权限就获得了提升，而对应的程序则由此获得了管理员的权限。
...
有时，我们会遇到一些程序报错，提示【请联系你的管理员】【请联系 Windows 管理员】等信息。这种说法是多用户时代的遗留——在那个时候，为了防止普通用户不小心破坏电脑的关键部分，供多人使用的电脑往往会禁止普通人进行「提权」，而能够提权的用户由专人管理。那时，如果我们需要安装一些软件，或者需要调整系统的关键设置，就需要线下联系他们来进行。这就是程序提示「请联系管理员」的原因。在今天，我们的个人电脑不再被多人使用，而程序所称的「管理员」，就是我们自己。
