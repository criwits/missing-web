...
为了和「已经绑定了微软账号的用户」区分，我们把原先的传统的「用户」称为「本地账户」。将本地账户链接到微软账号后，微软账号的登录密码就会变成我们开机使用的密码（除此之外还可以设置额外的数字密码），微软账号的用户名就变成了我们**显示在屏幕上**的用户名[^3]。如果你原本使用的是本地账户，打开系统设置→【帐户】→【帐户信息】，选择【改用 Microsoft 帐户登录】，按提示登录或者注册一个微软账号即可完成绑定。
