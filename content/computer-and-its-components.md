...
- 英特尔公司现在主要的 CPU 产品线称作「酷睿」（Core），而「酷睿」系列又分成了三个子系列，每个子系列都有不同档次的产品。
  ...
  | 说明 | 传统的酷睿系列，自 2008 年起沿用至今 | 2023 年发布的新系列，主要应用在高端产品上，增加了针对 AI 应用的优化|2024 年发布的新系列，针对笔记本电脑、嵌入式设备等产品进行了优化|
...
{{<hint warning>}}
注意：部分出售廉价台式机整机的商家在介绍产品时，会用「i7 级」「i9 级」之类的幌子来博人眼球，实际使用的是淘汰多年的旧 CPU 系列。在选购非品牌的台式电脑整机时请特别留意，如下图所示。

![「i9 级」CPU](computer-and-its-components/i9_Level.png?updated=20260207#center)
{{</hint>}}

我们常常说一台电脑是「双核」「四核」的，这里的「双核」「四核」就是处理器中的概念。今天，几乎所有 CPU 都在芯片中安装了多个「核心」，相当于一个个协同起来的独立的小处理器。例如，「双核」意味着在一枚处理器芯片上集成了两个核心，相当于两个大脑协同工作，当我们需要用电脑同时做很多事情的时候就有所裨益。同理，「四核」「八核」就是在一个芯片上集成了四个甚至是八个核心。现在，一些 CPU 还使用「大小核」设计，混合使用大小两种规格不同的核心，来实现性能和功耗之间的平衡。下图中，左方是一枚四核 CPU 示意图，右方则是一枚采用大小核设计的六核处理器示意图（图片不反映实际比例和结构）。

![多核示意图，大小非实际比例](computer-and-its-components/Multicore.png?updated=20251010#center)

然而，核心数只能反映「脑子多不多」，但无法刻画「一个脑子有多快」。要衡量一颗 CPU 的性能，除了直观地根据厂商划定的这些低中高端系列之外，「主频」也是一个重要的参考。所谓「主频」指的是这 CPU 一秒钟能够「工作」的次数，人们通常使用单位「GHz」来标注它。1 GHz 就表示「1 秒能『工作』10 亿次」。CPU 通常有一个「基准频率」和「加速频率」，前者是 CPU 正常工作时的稳定频率，后者则是在短时间内应对复杂任务时能达到的最高频率。目前，常见的台式机 CPU 的基准频率和加速频率能达到 3 GHz 和 5 GHz，而笔记本则在 2 GHz 和 4 GHz 左右。

在现实生活中，人们可以根据任务的耗时和性质来统筹安排时间，从而提升整体效率。例如，我们可以趁洗衣机工作时完成其他的家务。一些 CPU 核心也有这样的能力——通过合理分配任务执行的方式，单个 CPU 核心也有机会同时处理多项任务。这种技术被称为「超线程」。如果一颗 CPU 标称「8 核 16 线程」，就意味着它每个核心都有同时处理两个任务的潜力，在面临多任务场景时往往有更好的表现。

> 现在，许多多核 CPU 同时拥有单线程和多线程（通常是双线程）核心。你可以看看自己电脑 CPU 的核心数和线程数（见后文），试着计算一下每个核心的线程情况。

需要强调的是，**并不是说核心数、线程数越多，主频越高的处理器性能一定越好**，更**不是说 i7 处理器就一定比 i5 更好**，也**不是说英特尔和 AMD 有孰优孰劣之分**。我们应该综合理解这些概念：每个品牌都会随着时间推移一代代地更新，每一代都有着的不同系列，有的系列高端，有的系列低端；每个系列也都有自己的不同型号，有的型号性能强，有的型号性能弱。CPU 的性能并不与某一个因素呈线性的关系，而是多个因素叠加的结果。

按 `Ctrl` + `Shift` + `Esc` 打开「任务管理器」，选择【性能】页面，就能看见 CPU 的型号、基准频率（标注为「基准速度」）、核心数（标注为「内核」）以及总线程数（标记为「逻辑处理器」）。

> 你也可以右击【开始】按钮，选择【任务管理器】来打开「任务管理器」。

![在任务管理器中查看 CPU 属性](computer-and-its-components/Check_CPU.png#center)
...
> 所以，你电脑的所有资料都存在硬盘里。想直接拿走你的资料，应该拆下硬盘，而不是带走显示器或别的什么东西。
...
> 
> ![勾选【计算机】](computer-and-its-components/Select_This_PC.png#center)
> 
> 这样，你的桌面上就会显示那些对应的图标了。

**硬盘对电脑使用体验的影响，主要是打开软件的速度，包括开机的速度**。这是很容易理解的，因为数据和软件本身原先都是存在硬盘里的，处理器从硬盘里取数据的速度就直接影响着软件启动或者说加载的时间。

> 手机用「闪存芯片」来存储数据，如照片、视频和 app，它们类似于固态硬盘中的存储芯片。然而，某些手机厂商或商家常将其称为「内置存储」「存储内存」，甚至直接简称「内存」。显然，闪存**完全不是**真正的内存，日常生活中所说的「手机内存不够」，其实大多指的是闪存空间不足，即「手机的硬盘」容量不够，而不是真正意义上的内存不足。这是人们常将内存与硬盘混淆的根源之一。
...
- **集成显卡**，简称「**集显**」，一般也称「**核显**」，英特尔称「核芯显卡」，AMD 则将 GPU 和 CPU 构成的整体一同称为「APU」。GPU 被安排在处理器的同一片芯片上，性能相对较差，能应付大多数工作，但游戏、制图等特定工作就不太行了。集成显卡功耗很低，而且省去了用户单独购买显卡的麻烦。
...
![英伟达官方商城的 RTX 4070 Super](computer-and-its-components/4070_storepage.png)
