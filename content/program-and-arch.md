...
在 [万言皆数——字符与编码规则]({{ref "characters-and-encodings.md"}}) 一章中我们提到，计算机中的一切信息都由电路承载；具体地说，每个信息都是由若干位 0 和 1 这样的二进制数字所组成，反映在电路上，就是「有电」「无电」之间的排列组合。为了实现程序在硬件上运行，自然而然地，我们首先就需要寻找「程序」在硬件上的表达方式。具体来说，是程序的每一步用 0 和 1 的表达方式。

在本章的第一节，我们构想了一个「番茄炒蛋」的程序，不过那时我们认为这个程序的「硬件」就是我们人类。现在，我们想要让一台炒菜机器也能执行这份菜谱程序。这种简单的炒菜并不能看懂中文、英文这样的人类语言，它所能做的，就是不断地读取某处并排的 8 根导线上的信号，然后根据这组信号的值，依据固定的规则，执行一个简单的动作。假设这台炒菜机器的部分设计如下：

- 机器一共有 4 个用来装食材的碗，可以在烹饪过程中使用。
- 机器不断地读取并排的 8 根导线上的信号，并按照下面的规则选择动作：
  - 当输入的信号是 `0001xxyy` 时，机器会执行「洗一份食材」的动作，`xx` 决定食材的种类：`00` 代表番茄、`01` 代表青椒……而 `yy` 表示洗完之后的食材加在哪个碗里，一共有 `00` 至 `11` 四个碗可以使用。例如，信号 `00010000` 能控制机器洗一份番茄放到第 0 号碗里。
  - 当输入的信号是 `001000yy` 时，机器会执行「打鸡蛋」的动作，`yy` 决定打好的蛋放在哪个碗里，同样地，可以选择从 `00` 至 `11` 的四个碗。
  - 当输入的信号是 `001100yy` 时，机器会执行「切菜」的动作，将 `yy` 号碗中的食材全部切块。
  - 当输入的信号是 `01000xxx` 时，机器会给灶台点火，并向锅中加入 {{<katex>}}10\times\bar{xxx}_2{{</katex>}} mL 的油。例如，`01000100` 对应的 `xxx` 是 `100`，这组信号会让机器点火后加入 {{<katex>}}10\times4=40{{</katex>}} mL 的油。
  - 当输入的信号是 `010010yy` 时，机器会向锅中倒入碗 `yy` 中的全部内容。
  - 当输入的信号是 `1000xxxx` 时，机器会在锅中翻炒 {{<katex>}}10\times\bar{xxxx}_2{{</katex>}} 秒。
  - 当输入的信号是 `1100xxxx` 时，机器会等待 {{<katex>}}10\times\bar{xxxx}_2{{</katex>}} 秒。
  - 当输入的信号是 `1110xxxx` 时，机器会向锅中加入 `xxxx` g 的盐。例如，`11100101` 对应的 `xxx` 是 `101`，这组信号会让机器向锅中加入 5 g 盐。
  - 当输入的信号是 `111100yy` 时，机器会将锅中的所有内容倒到 `yy` 号碗中。
  - 当输入的信号是 `11111111` 时，机器会熄灭灶台。

那么，我们可以依次给这台机器发送下面的 24 组信号，机器就能完成菜谱的全部内容。
