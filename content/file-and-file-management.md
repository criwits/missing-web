...
当收到一个压缩文件时，我们一般需要将它解压，来还原出原始的文件。如果你电脑上已经安装有压缩工具，那么可以右击压缩文件，选择类似「解压到当前文件夹」或者「解压到 `<文件名>\`」[^5]的选项。这两类选项的不同是：
...
当你想制作一个压缩文件时，可以先选中你要打包的所有文件和文件夹，右击它们并选择类似「新建压缩包」「添加到归档文件」或「压缩为 `<文件名>`」的选项。压缩工具可能会提示你设置一些参数，例如压缩格式（推荐 `zip` 格式）、压缩后的文件的文件名以及压缩方式。
...
- 如果电脑感染了恶意的木马病毒，一些文件的打开方式也可能被偷偷修改。
...
双击桌面上的【此电脑】，展开【文件夹】折叠项就能看到这些文件夹：
...
默认情况下，**已知文件夹的实际位置在 `C:\用户\<你的用户名>`[^users]，这是你的「用户配置文件夹」（user profile folder，俗称「用户文件夹」）**。除了直接在【此电脑】中查看外，你也可以手动打开这个路径来找到它们。已知文件夹涵盖了「桌面」「文档」「图片」「音乐」「视频」等多种类别，并且都配有形象的图标。如果你计划将自己的文件存放在 C 盘，这些目录是不错的选择：毕竟，C 盘作为 Windows 的系统分区，有严格的保护机制，最好不要把你的文件在 C 盘随意存放。

[^users]: `C:\用户` 文件夹有些特殊：它在资源管理器中显示的名称是「用户」，但硬盘中存储的名称实际上是 `Users`。因此，你可能会在其他地方看到 `C:\Users` 这样的写法。各种已知文件夹也有类似的机制，比如「文档」的真实文件夹名是 `Documents`。在本书中，为了方便你在资源管理器中直接找到它们，我们会统一采用显示名称。

但如果你打算把自己的文件放到其他的分区，你的选择就会自由很多，各个分区的根目录都是不错的位置。你可以仿照 Windows 给你的已知文件夹，自己在其他分区的根目录下按类别新建好「文档」「电影」「音乐」等文件夹，用来帮助自己整理文件。或者更直接点，**你还可以将这些已知文件夹迁移到其他分区**，这样它们的实际位置就不再是 `C:\用户\<你的用户名>`，也不会占用 C 盘的空间了。下面我们给出迁移操作的步骤。
...
- 有些软件会使用系统的已知文件夹存储数据，但不会自动跟随已知文件夹迁移。例如，QQ 默认将聊天记录保存在 `C:\用户\<你的用户名>\文档\Tencent Files`，但如果你将「文档」迁移到其他位置，QQ 不会随之修改，仍然会使用 `C:\用户\<你的用户名>\文档`。这时，**需要手动在软件中修改数据存储路径，指向新的位置**。
{{</hint>}}

- 首先，我们需要在迁移的目标位置创建一个新的文件夹，作为已知文件夹新的「本体」。比如，如果我们想将已知文件夹「文档」迁移到 D 盘，就新建一个 `D:\Documents` 目录。迁移完成之后，它将成为新的「文档」文件夹。为了保障兼容性，这一步建议使用有意义的简短英文名称，如 `Documents`、`Downloads` 或是 `Video`。

- 在资源管理器中，右击某个我们想要迁移的文件夹（比如【文档】），选择【属性】，然后切换到【位置】选项卡，如图：

    ![查看「文档」的位置](file-and-file-management/Pos_of_Documents.png#center)

- 我们将中间文本框里的路径，改成第一步创建好的目标路径，在本例中就是 `D:\Documents`[^6]。

- 点击【应用】，系统提示「是否要将所有文件从原位置移动到新位置」，选择【是】。
...
对于系统分区 C 盘来说，你不去动它，它也会被塞入一些诸如系统临时文件、Windows 更新文件等等这些。诸如「火绒安全软件」「360 安全卫士」之类的工具通常都会提供「系统清理」功能，可以帮助你清理这些文件。除此之外，Windows 自身也提供了一个磁盘清理工具。在 [成为空间管理大师]({{<relref "manage-storage.md">}}) 一章中，你将学习到如何使用它来打扫你的 C 盘。
...
[^6]: 修改时请注意不要填错，更不要在此处填入某个分区的根目录。如果你不小心把一个已知文件夹的位置设置成了某个磁盘的根目录（比如说 `D:\`），可能很难再改回来。