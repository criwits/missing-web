...
在本章的后续部分，以及 [软件的寻找与安装]({{<ref "software-installation.md">}}) 中，我们会详细介绍，怎样把我们的软件以及其他东西放在 C 盘以外的地方，以及这样做的其他好处。如果你的 C 盘空间已经不足，那么请参考 [成为空间管理大师]({{<ref "manage-storage.md">}}) 中的方法，来释放 C 盘的空间。

> 如果你打开「此电脑」之后，发现自己电脑上只有一个 C 盘，这说明你的电脑磁盘只有一个分区。一些品牌的新电脑出厂时就是这样的设置。如果你想要分出一个新的分区，可以参考 [成为空间管理大师]({{<ref "manage-storage.md#用-diskgenius-进行高级分区调整">}}) 中的方法。
>
...
每次我们启动「网易云音乐」，运行的都是 `cloudmusic.exe` 这个可执行文件。然而，这个文件的运行离不开放在它边上的那一堆辅助文件——如果你把 `cloudmusic.exe` 复制到另外的一个地方，双击运行，大概率会直接报错；即使不报错，功能也必不完全正常。
...
- Windows 的各种 bug。例如，Windows 会时不时用 Microsoft Edge 代替其他软件来打开网页文件和 `pdf` 文件等。 ~~（微软你坏事做尽）~~
...
> 如果你打开「此电脑」后，发现自己电脑只有一个 C 盘，这意味着你的电脑磁盘只有一个分区，一些品牌的电脑出厂时就是这样的。这种情况下，你可以先跳过这一小节，或者参考 [成为空间管理大师]({{<ref "manage-storage.md#用-diskgenius-进行高级分区调整">}}) 中的方法，将 C 盘的空间划分成两个分区，再进行这一小节的操作。

上文提到了一个叫做 `文档` 的文件夹，它是系统预置的一个用户文件夹。在介绍这一部分之前，我们先简单提一下「用户文件夹」是什么。

如前所言，Windows 系统默认为用户准备了几个文件夹来进行文件的分类。对于 Windows 11，你可以在「此电脑」窗口的左侧看到这些文件夹。
...
**强烈建议迁移的用户文件夹有：「文档」「桌面」「下载」；建议迁移的用户文件夹有：「视频」「图片」「音乐」。** 迁移完成之后，你可以在迁移之后的位置（比如 D 盘）看到这些文件夹。它们现在不在原来的 `C:\用户\<你的用户名>\` 那里了。
