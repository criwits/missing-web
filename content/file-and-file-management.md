...
默认情况下，**已知文件夹的实际位置在 `C:\用户\<你的用户名>` 下，这是你的「用户配置文件夹」（User Profile Folder，俗称「用户文件夹」，又称「主目录」）**。除了直接在【此电脑】中查看外，你也可以手动打开这个路径来找到它们。已知文件夹涵盖了「桌面」「文档」「图片」「音乐」「视频」等多种类别，并且都配有形象的图标。如果你计划将自己的文件存放在 C 盘，这些目录是不错的选择。
