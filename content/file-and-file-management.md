...
- 一个是「**盘符**」，盘符是一个英文字母加上冒号 `:` 构成的。我们所称呼的「C 盘」「D 盘」正是指盘符中的那个字母。如上图，左方分区的盘符是 `C:` ，右方分区的盘符是 `D:` 。盘符一般在被指定之后就不方便更换了。在今天，Windows 系统中的盘符都是从字母 C 开始¹的。
...
> 如果你打开「此电脑」之后，发现自己电脑上只有一个 C 盘，这说明你的电脑磁盘只有一个分区。一些品牌的新电脑出厂时就是这样的设置。如果你想要分出一个新的分区，可以参考 [成为空间管理大师]({{<ref "manage-storage.md">}}) 中的方法。
>
...
- 「点号」是指文件名中间的那个 `.` 。
- 「扩展名」是指文件名中，点号 `.` 之后的部分²。扩展名展示着文件的类型，它会告诉操作系统，这个文件应该用什么方式来打开。例如，上图中 `本科毕业论文（设计）写作指南（理工类）.doc` 中的 `doc` 就是这个文件的扩展名，它说明这个文件是一个「Word 文档」，应该使用 Word 或 WPS 等软件打开。
...
「快捷方式」可以看成某个具体文件的单向「指针」或者说「替身」，指向电脑某个角落里的某个文件。它的扩展名是 `lnk`，但实际上不可见³。你桌面上的「网易云音乐」，指向的正是网易云音乐软件目录下的那个 `cloudmusic.exe` 文件。右击桌面上的【网易云音乐】，选择【属性】，会弹出这个快捷方式的详细信息：
...
因此，卸载软件并不能仅仅把桌面上的「快捷方式」删掉，而需要通过卸载工具进行。我们将在 [基本维护与安全防护]({{<ref "basic-maintenance.md">}}) 中介绍。
...
压缩文件有很多种类。最常用的是 `zip` 文件和 `rar` 文件，但后者的压缩软件是收费的⁴。我们建议在与他人交换文件的时候，只使用 `zip` 格式打包。
...
- 右击压缩文件，选择【解压⁵到当前文件夹】或者【解压到 `<文件名>\`】。这两个选项的不同是：
...
> 如果你打开「此电脑」后，发现自己电脑只有一个 C 盘，这意味着你的电脑磁盘只有一个分区，一些品牌的电脑出厂时就是这样的。这种情况下，你可以先跳过这一小节，或者参考 [成为空间管理大师]({{<ref "manage-storage.md">}}) 中的方法，将 C 盘的空间划分成两个分区，再进行这一小节的操作。
...
{{< details title="操作速查卡：更改用户文件夹的存储位置" open=true >}}
