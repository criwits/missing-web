...
{{<hint info>}}
在这一部分，我们将重新介绍你可能所不知道的「文件」以及文件管理的技巧。看完这一部分，你将可以找到下面这些问题的答案：
...
如上一章所言，硬盘是电脑中存放数据的地方，而「文件」则是数据存放的具体形式。你所撰写的 Word 文档、制作的 PPT 幻灯片、从网上下载的图片，乃至各个软件本身，都以文件的形式存储在硬盘上。

这一部分，我们将具体介绍「文件」和文件的管理。这是《你缺计课》需要动手实践的第一章，更是我们合理、有效地使用电脑的第一课。

## 硬盘的分区

双击桌面上的【此电脑】，就能打开「文件资源管理器」。在其中，我们可以看到一个或几个「盘」，例如 C 盘、D 盘等。这样的「盘」学名叫做「分区」，顾名思义，它们是将硬盘上的空间人为地划分成了一些子空间。
...
这一节我们介绍一种类型特殊的文件——「可执行文件」。我们提到，数据是以文件的形式存储在硬盘上的，比如你所撰写的 Word 文档，它们都存储成了扩展名为 `doc` 或者 `docx` 的文件；你所下载的图片，它们的扩展名则往往是 `jpg` 、 `png` 或者 `gif` 。而我们又提到，软件（即 app）也是以文件的形式存储在硬盘上的。那么，软件本身是什么格式的文件呢？
...
因此，双击打开这个「网易云音乐」快捷方式，就等同于打开 `cloudmusic.exe` 这个文件。如果你**删掉了这个快捷方式，它并不会影响 `cloudmusic.exe` 这个文件，不会影响你电脑上安装的网易云音乐本身**。因此，卸载软件并非仅仅把桌面上的「快捷方式」删掉就完事了，而是需要通过借助卸载工具。我们将在 [基本维护与安全防护]({{<relref "basic-maintenance.md">}}) 中介绍。
...
- 对自己的文件进行整理归类。比起将所有文件随手乱放在桌面上，如果我们将自己的文件按它们的性质、类别等归类存放，必然有助于提升我们的工作效率。
...
- 如果你的电脑只有一个 C 盘（很多新机如此）：
  - 若你的总容量小于 300 GB，建议不要再分额外的分区，将你的文件直接存放在 C 盘。同时，你应当使用移动硬盘或各种「网盘」来备份好你的重要数据。
  - 如果你的总容量在 300 GB—500 GB，可以选择为 C 盘留下 200 GB 左右，并将剩余的空间分出一个新的分区（D 盘）。然后，在 D 盘存放你的文件。
  - 如果你的总容量在 500 GB 以上，建议为 C 盘留出 400 GB—500 GB 的空间，再将剩余的空间按自己的喜好，分出一个或多个新的分区（D 盘、E 盘……）。然后，在这些分区存放你的文件。
- 如果你的电脑已经有多个分区（C 盘、D 盘……），可以先在「任务管理器」中，查看每个分区对应的硬盘：
  - 如果你只有一块硬盘，通常这种情况下，你的 C 盘不会很大，建议直接在 D 盘和之后的分区存放你的文件。
  - 如果你有多块硬盘，并且所有硬盘都是 SSD，我们亦建议你使用 D 盘和之后的分区存放文件。
  - 如果你有多块硬盘，并且有的分区在 SSD 上，有的分区在 HDD 上，我们建议你将常用的软件和数据放在位于 SSD 的分区上（包括 C 盘），而将其他大型软件和数据放在位于 HDD 的分区上。
...
为了给文件安家，我们首先需要挑选一个合适的位置。**如果你打算把文件放在 C 盘，可以使用 Windows 为你提供的一系列「用户文件夹」**。一般这些文件夹可以在【此电脑】窗口中看到：
...
![Windows 10 用户文件夹](file-and-file-management/User_directories.png#center)

{{% /tab %}}
{{< /tabs >}}

默认情况下，这些文件夹的实际位置在 `C:\用户\<你的用户名>` 下。除了直接在【此电脑】中查看外，你也可以手动打开这个路径来找到它们。这些文件夹涵盖了「桌面」「文档」「图片」「音乐」「视频」等多种类别，并且都配有形象的图标。如果你计划将自己的文件存放在 C 盘，这些目录是不错的选择。

{{<hint warning>}}
注意：C 盘作为 Windows 的系统分区，有严格写保护机制，最好不要把你的文件直接放在 C 盘根目录下。
{{</hint>}}

如果你打算把自己的文件放到其他的分区，各个分区的根目录都是不错的选择。**你可以仿照 Windows 给你的用户文件夹，自己在其他分区的根目录下按类别新建好「文档」「电影」「音频」等文件夹**，用来帮助自己整理文件。更进一步，**你还可以直接将 Windows 提供给你的这些用户文件夹「迁移」到其他分区**，这样它们的实际位置就不再是 `C:\用户\<你的用户名>`，而是你指定的地方了。例如，如果你想使用 D 盘根目录即 `D:\` 来存放这些用户文件夹，可以按下面的步骤操作。

- 右击某个我们想要迁移的文件夹（比如【文档】），选择【属性】，然后切换到【位置】选项卡，如图：
...
一般来说移动操作很快就会完成。完成后，点击【确定】。这样「文档」文件夹就被成功地迁移到了 D 盘，你可以直接在【此电脑】中双击【文档】打开它，也可以打开 `D:\文档` 来访问，如下图所示。**如果你打算使用 C 盘以外的分区存放自己的文件，我们强烈建议你迁移各用户文件夹到对应的分区**，因为它们能帮你更好地管理文件。
...
- 作为一个用户文件夹，它默认在 C 盘，如果你没有按照前文迁移它的话。各大浏览器、新版 QQ、飞书等应用都会把接收到、下载到的文件存在这里；
...
> 这也解释了为什么我们在前文中提到，如果你决定使用其他分区存放文件，就最好将这些用户文件夹一并迁移。因为许多软件都会默认使用它们来存放数据，将它们迁移走可以更好地缓解 C 盘压力。
