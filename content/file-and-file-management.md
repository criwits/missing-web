...
默认情况下，**已知文件夹的实际位置在 `C:\用户\<你的用户名>`[^users]，这是你的「用户配置文件夹」（user profile folder，俗称「用户文件夹」）**。除了直接在【此电脑】中查看外，你也可以手动打开这个路径来找到它们。已知文件夹涵盖了「桌面」「文档」「图片」「音乐」「视频」等多种类别，并且都配有形象的图标。如果你计划将自己的文件存放在 C 盘，这些目录是不错的选择：毕竟，C 盘作为 Windows 的系统分区，有严格的保护机制，最好不要把你的文件随意存放在 C 盘。

[^users]: `C:\用户` 文件夹有些特殊：它在资源管理器中显示的名称是「用户」，但硬盘中存储的名称实际上是 `Users`。因此，你可能会在其他地方看到 `C:\Users` 这样的写法。各已知文件夹也有类似的机制，比如「文档」的真实文件夹名是 `Documents`。在本书中，为了方便你在资源管理器中直接找到它们，我们会统一采用显示名称。
...
- 有些软件会使用已知文件夹存储数据，但不会自动跟随你的操作而迁移。例如，QQ 默认将聊天记录保存在 `C:\用户\<你的用户名>\文档\Tencent Files`，但如果你在安装 QQ 后将「文档」迁走了，QQ 仍旧会使用 `C:\用户\<你的用户名>\文档`。这时，**我们需要手动在软件中修改数据存储路径，指向新的位置**。
{{</hint>}}

- 首先，我们需要在迁移的目标位置创建一个新文件夹，作为已知文件夹的新「本体」。比如，如果我们想将「文档」迁移到 D 盘，就可以新建一个 `D:\Documents` 目录。迁移完成之后，它将成为新的「文档」文件夹。为了避免某些潜在的问题，这一步建议使用简短而有意义的英文名，如 `Documents`、`Downloads` 或是 `Video`。
...
[^6]: 修改时请注意不要填错，更不要在此处填入某个分区的根目录（比如说 `D:\`）。如果你不小心把一个已知文件夹的位置设置成了某个磁盘的根目录，可能很难再改回来。