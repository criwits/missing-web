...
页面文件的作用，是给系统提供一种「虚拟内存」：当电脑的内存不足以容纳打开的全部软件时，操作系统会将一内存中暂时未用到的一部分数据存入硬盘，从而腾出空间给正在使用的数据和程序。而当我们需要用到那些被存入硬盘的数据时，操作系统再将它们取回内存，同时从内存中取出另一部分近期未用的数据换入硬盘。由于这个交换以一定大小的「页」为单位，我们把硬盘中负责暂存内存数据的文件就称为「页面文件」。在 Windows 系统中，页面文件的默认位置，就是 `C:\pagefile.sys`（系统级隐藏，所以看不见）。它的大小因机器的内存而异，从几 GB 到几十 GB 都有可能。

而休眠文件的作用同样与内存有关。「休眠」（Hibernate）是电脑的一种工作状态，在这种状态下全机断电，但是上电后可以快速恢复到休眠前的状态。由于内存断电就丢失数据的特性，当需要休眠时，操作系统要先将内存里的数据全部存入硬盘——而这休眠文件，就是硬盘上用来接收内存数据的地方，默认位于 `C:\hiberfil.sys`。同样，它也是系统级隐藏的，在资源管理器中看不见它。它的大小也有大有小，一般都会达到数个 GB。
...
页面文件、休眠文件的大小是可以调整的，你可以自行上网搜索相关的方法。不过我们不建议自行贸然调整它们——操作系统会自动地调节它们的大小，而对它们随意调整，可能对电脑的性能存在影响。
