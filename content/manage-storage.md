...
再打开你的用户文件夹，它位于 `C:\Users\<你的用户名>`，这里有一个隐藏的 `AppData` 文件夹，是存放所有软件用于你这个用户的数据，整个用户文件夹中「看不见的内容」大多都在这里面。它里面有三个子文件夹：
...
页面文件、休眠文件的大小是可以调整的，你可以自行上网搜索相关的方法。不过我们不建议自行贸然调整它们——操作系统会自动地调节它们的大小，而对它们随意调整，可能对电脑的运行存在影响。
