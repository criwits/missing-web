...
你如果留心观察过软件的安装过程，或者仔细阅读了 [软件的寻找与安装]({{<ref "software-installation.md#安装软件">}}) 一章，就会知道 `Program Files` 与 `Program Files (x86)` 两个文件夹是用来存放软件本体的文件夹。不过，实际上它们存放的是「为所有用户安装的软件」，至于只为一个用户安装的软件，我们后面再说。那么，为什么要分两个文件夹呢？这还要从 CPU 说起。

<!--这下面的内容之后请移动到「体系结构」一章-->
