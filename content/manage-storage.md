...
- 第一个分区显示为「ESP」，它是「EFI 系统分区」（EFI[^EFI] system partition）的简称。简而言之，这个分区提供启动操作系统的信息，让你的电脑知道该在哪启动 Windows。此分区不可随意操作。
- 第二个分区显示为「MSR」，它是「微软保留分区」（Microsoft Reserved Partition）的简称，用来为曾经的老式分区结构提供兼容支持。全新安装的系统可能没有这个分区，但如果存在，则必须位于操作系统分区之前。此分区不可随意操作。
- 第三、第四和第五个分区分别是 C 盘、D 盘和 E 盘三个普通分区，也是我们可以调整空间的分区。
...
> 在磁盘管理工具中，ESP 和恢复分区通常也会显示（没有盘符），但不可操作。
...
现在我们的分区结构是：C 盘（约 200 GB）、D 盘（约 160 GB）和 E 盘（约 160 GB）。现在，让我们来尝试在 E 盘「切」出 40 GB，然后将它跨过 D 盘、E 盘，分配给 C 盘。
在前文提到，为了实现这样的操作，我们需要将 D 盘和 E 盘的数据后移，把空闲空间移到 C 盘后面，再将 C 盘向后扩大。幸运的是，在 DiskGenius 中，已经提供了方便地跨分区空间合并功能——软件会自动帮助我们实现数据的后移。

右击 E 盘，选择【调整分区大小】，接着，软件会弹出这样的界面：
...
我们的第一个任务是在 E 盘「切」出空闲空间。虽然前文提到，这种操作只能在分区尾部完成，但 DiskGenius 却提供了「在分区前部『切』出空闲空间」的功能——其实，它的本质仍然是在尾部操作，随后将数据后移罢了。为了实现这一操作，我们按住蓝色条条的左端，向右拖动，拖出合适大小的空闲空间即可。如果要精确指定分区的容量，可以在底下的参数「分区前部的空间」中直接输入你想要的数值。

![拖出空闲空间](manage-storage/Drag_Size.png#center)

有了空闲空间，我们就可以将它跨过多个分区，直接合并给 C 盘了。点击「分区前部的空间」右侧下拉框，在这里选择【合并到 本地磁盘(C:)】，然后点击【开始】，点两下【是】。
