...
现在，**所谓「训练模型」，就是求损失函数 {{<katex>}}L(a, b){{</katex>}} 的最小值点**。
...
事实上，如果函数满足某些特性，在某一局部存在最小值点，通过选择好调整合适的步长参数 {{<katex>}}\alpha{{</katex>}}，不断地「走」，就可以近似找到这个最小值点。这个过程即是「梯度下降」，而步长参数 {{<katex>}}\alpha{{</katex>}} 称为「学习率」，每走一步称为「训练一步」，而整个「寻找最小值」的过程，就是 AI 模型开发过程中最为耗时的过程之一——「训练」。
