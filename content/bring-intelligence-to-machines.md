...
现在，让我们回到本节开头的那个问题——为什么今天大多数的 AI 都运行在远方的服务器上，而不是我们自己的设备上呢？答案显而易见——我们自己的手中的算力，通常不足以运行 AI 模型。我们已经知道，AI 对话模型基本上是「一字一词」地生成回复，算力不足最直观的体现，就是模型要几秒，甚至几分钟才能生成一个字。

以智谱清言所使用的 GLM 模型的开源版本 GLM-4-9b 为例，其拥有 90 亿参数，如果我们想使用 GPU 来运行它，需要近 20 GB 的显存（BF16 量化），而今天拥有这么多显存的显卡均价格不菲。又比如当下大火的 DeepSeek 模型，其「满血版」拥有 6710 亿参数，需要同时使用多张显存巨大的专业计算卡（价格也更加昂贵）运行，这显然不是一般人所能承受的。

为了减少显存的占用，人们提出了「量化」的思路，通过降低参数的精度来减小存储它们所需要的体积，前文提到的「BF16 量化」就是一个例子。然而，即使使用了量化技术，AI 模型对显存的需求依然不可小觑。同时，即使不考虑显存的需求，AI 对显卡本身的性能也提出了相当大的考验——毕竟，一个每秒只能生成半个字的 AI 助手，观感上就不是很好，更别说实用了。这些因素让 AI 的「本地部署」困难重重。

不过，尽管大型模型的本地部署相当困难，但是「将 AI 模型本地部署」这事本身有着许多的好处：首先，本地部署解决了「随时使用」的问题。当网络连接不通畅或同时使用的人数过多时，那些在线的 AI 平台就会拒绝服务——在 2025 年初，国产 AI 模型 DeepSeek 大火，那时的 DeepSeek 就经常「服务器繁忙，请稍后再试」。如果将 AI 模型运行在自己的设备上，就完全不必担心这样的情况。同时，访问 AI 的延迟也能降低。此外，本地部署还可以有效防止数据外泄——假如你是某企业的负责人，如果你的员工每天都在借助网上的 AI 平台来解决工作问题，你是否会担心员工不小心将企业的敏感信息发给它们？通过将 AI 模型本地部署，就能实现数据的可控。

技术的发展，使得硬件的算力不断增强，而许多的手机、电脑处理器也开始额外集成 NPU，二者相辅相成之下，在本地部署中小型的 AI 模型已经初见应用。一方面，许多手机、电脑厂商都在采用「小模型本地部署 + 大模型在线服务」的思路，打造越来越通用的 AI 助手；另一方面，随着量化技术的进步，低端 GPU 甚至纯 CPU 运行 AI 模型的效率也会不断提升，即使你的电脑没有独立显卡，也能借助 CPU 与内存来运行一些开源的小型模型。如果你对这方面有兴趣，可以以「<模型名称> 本地部署」为关键词，在互联网上搜索相关的信息。

{{<hint danger>}}
警告：目前，几乎所有 AI 模型本地部署的框架都是开源软件——这意味着你不需付出任何软件成本就能使用它们。（当然，设备还是得自己买的。）**所有诱导你「付费体验本地部署模型」的（无论是 DeepSeek、Llama、GLM 还是别的什么），都是诈骗。**
{{</hint>}}
