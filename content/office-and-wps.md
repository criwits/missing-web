...
尽管新格式看起来扩展名只是多了个字母 `x`，但是与旧版格式是**完全不兼容**的。Office 2007 以及之后的各版本，可以打开新旧两种格式的文档，并且在保存文件时可以选择保存为新旧两种格式中的某一种。下图中，上方「Word 文档 (\*.docx)」便是新格式，而下方「Word 97-2003 文档 (\*.doc)」则是旧格式。
...
然而，作为国产软件的 WPS，自然有着一系列国产软件特有的问题——例如，广告和「不登录不给用」。在上一小节的 WPS 主界面截图中，可以看到整个 Ribbon 区域的按钮都呈灰色状态，下方还有显眼的登录提示。只有在注册并登录一个金山办公账号之后，才能使用完整的应用功能。对于那些注重数据私密性的用户来说，这无疑是一个极大的缺点。
